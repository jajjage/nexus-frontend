...
export function DataPlanCard({ product, onClick }: DataPlanCardProps) {
  ...
  // const hasBonus = Math.random() > 0.7; // REMOVED: caused flickering
  const hasBonus = product.id.charCodeAt(0) % 5 === 0; // Stable mock based on ID
  const bonusText = "Voice Bonus";

  // 1. Format Data Volume
  // Use `dataMb` from API if available, otherwise parse from name
  const formatDataVolume = (mb: number | null, name: string) => {
    if (mb) {
      return mb >= 1000 ? `${mb / 1000} GB` : `${mb} MB`;
    }
    // Fallback: Extract from name (e.g. "MTN 100MB Data")
    const match = name.match(/(\d+(\.\d+)?)\s?(GB|MB|TB)/i);
    return match ? match[0].toUpperCase() : name;
  };

  const displayVolume = formatDataVolume(product.dataMb, product.name);
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 mb-1">
        <h3 className="text-foreground text-2xl font-bold">{displayVolume}</h3>
      </div>
