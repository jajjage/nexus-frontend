...
const handlePlanClick = (product: Product) => {
  ...
  if (detected && !product.operator.name.includes(detected)) {
    toast.error(
      `Mismatch: Number is ${detected}, plan is ${product.operator.name}`
    );
    return;
  }
...
export function PaymentFlowManager({ product, phoneNumber, onClose }) {
  const [step, setStep] = useState<
    "checkout" | "biometric" | "pin" | "success" | "failed"
  >("checkout");
  ...
  return (
    <>
      ...
      <PinModal isOpen={step === "pin"} onSuccess={(pin) => callApi(pin)} />

      <SuccessModal isOpen={step === "success"} onShare={shareReceipt} />
    </>
  );
}
```
